...
static SDCardType sd_get_type(SdSpi *sdspi) {
	assert(sdspi);
...
int sdspi_write(int uid, int blk, void *buf, uint32_t count) {
	return -EBADF;
}


static ssize_t sdspi_read(dev_t device, off_t pos, void *buf, size_t count) {
	...
	if (memblk[blk].len < pos + count) {
		if (memblk[blk].len < pos)
			return -EIO;
		count = memblk[blk].len - pos;
	}

	if ((block = sd_get_card_size(sdspi)) < 0)
		return blocks;

	return blocks * SDSPI_BLKSIZE;
}


static ssize_t sdspi_blksize(dev_t device) {
	return SDSPI_BLKSIZE;
}


static ssize_t sdspi_devsize(dev_t device) {
	Device *dev;
	SdSpi *sdspi;

	if (!(dev = device_lookup(dev_t device_number)))
		return -ENODEV;
	
	if (!(sdspi = dev->priv))
		return -ENODEV;

	//TODO: figure out if device really belongs to sdspi
...
int sdspi_open(SpiContext *spi) {
	Device *dev = NULL;
	SdSpi *sdspi = NULL;
	int d, ret;

	if (!(sdspi = kmalloc(sizeof(SdSpi))))
		goto fail;

	if (!(dev = kmalloc(sizeof(Device))))
		goto fail;

	dev->type = DEVICE_TYPE_BLOCK;
	dev->blockdev.read = sdspi_read;
	dev->blockdev.blksize = sdspi_blksize;
	dev->blockdev.size = sdspi_devsize;
	dev->priv = sdspi;

	ret = device_register("memcard0", dev, &sdspi->device);

	if (ret < 0)
		goto fail;
	else
		kprintf(LOG_LEVEL_INFO, "[memblk] Registered memblk0 at %i, %i [@0x%X]\n", major(dev->device), minor(dev->device), &dev);

	return ret;

fail:
	kprintf(LOG_LEVEL_ERROR, "[sdspi] Failed to register sdspi device\n");
	kfree(sdspi);
	kfree(dev);
	return -ENOMEM;
}
